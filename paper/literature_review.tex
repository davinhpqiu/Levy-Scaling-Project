...
Samorodnitsky and Taqqu \cite{SamorodTaqqu1994}, under which the characteristic function of a
...
\cite{SamorodTaqqu1994,GnedenkoKolmogorov1954}. A distribution
...
mathematical theory are provided by Samorodnitsky and Taqqu \cite{SamorodTaqqu1994} and
Zolotarev \cite{Zolotarev1986}; the monograph of Uchaikin \cite{Uchaikin2003} covers
applications in physics and finance.

\subsection{Mandelbrot's stable hypothesis for asset prices}

Mandelbrot \cite{Mandelbrot1963} was the first to propose stable distributions as a
...
inconsistent with the Gaussian random-walk model of Bachelier \cite{Bachelier1900}.
...
The hypothesis was further developed by Fama \cite{Fama1965}, who examined
...
\cite{Weron2001}, a point we return to in Section~\ref{sec:estimation}.
...
parameter \cite{SamorodTaqqu1994}. Setting $x = 0$ and using
...
Mantegna and Stanley \cite{MantegnaStanley1994} introduced the \textit{truncated Lévy flight}
...
\cite{BouchaudPotters2003,Cont2001,Mantegna1991}, because it allows
...
A brief controversy arose when Copson and Smith \cite{CopsonSmith1995} and
Mantegna \cite{Mantegna1995reply} debated whether the S\&P 500 data in
Mantegna and Stanley \cite{MantegnaStanley1995} were truly consistent with a stable law;
the consensus view \cite{BouchaudPotters2003} is that the TLF
...
Mantegna and Stanley \cite{MantegnaStanley1994,MantegnaStanley1995}. Using high-frequency
...
Mantegna and Stanley \cite{MantegnaStanley1995} demonstrated that the PDF collapses onto a
...
Gopikrishnan et al.\ \cite{Gopikrishnan1998} and Gopikrishnan et al.\ \cite{Gopikrishnan1999} extended the
...
be required for stability). Weron \cite{Weron2001} showed, however, that a
...
Plerou et al.\ \cite{Plerou1999} showed that the scaling exponent is consistent across
a large cross-section of US equities, supporting the universality picture.
Gabaix et al.\ \cite{Gabaix2003} provided a theoretical foundation for the inverse-cubic
law based on the trading of large institutions.

More recently, Kadioglu et al.\ \cite{Kadioglu2021} applied return-distribution analysis to
...
data from 2010-2019. Masoliver and Perelló \cite{Masoliver2021} provided an updated analysis of
...
in the post-crisis period. Jiang et al.\ \cite{Jiang2019} found similar scaling behavior
...
Bouchaud et al.\ \cite{Bouchaud2002} documented heavy-tailed and approximately stable
...
Cont \cite{Cont2001} surveyed stylized facts across equity, FX, and futures
instruments, noting that heavy tails and volatility clustering are robust
across all asset classes. The recent survey Cont et al.\ \cite{ContMiami2023}
...
algorithmic and high-frequency trading. Masoliver and Perelló \cite{Masoliver2021} addressed this
...
deviation than in earlier data. Cont et al.\ \cite{ContMiami2023} confirmed the
...
Mantegna and Stanley \cite{MantegnaStanley1994}. It has the advantage of being entirely
...
Weron \cite{Weron2001} documented that log-log regression systematically
...
regression-based methods by Drozdz et al.\ \cite{Drozdz2003}. A closely related
``curve collapse'' approach has been used extensively in condensed-matter and
statistical physics to identify critical exponents \cite{Houdayer2004}.
...
Bouchaud and Potters \cite{BouchaudPotters2003} (Ch.~2) as a consistency check on the
...
to finance by Mandelbrot \cite{Mandelbrot1997} and Muzy and Bacry \cite{MuzyBacry1993} in the
...
slowly-varying corrections. Bouchaud and Potters \cite{BouchaudPotters2003} (Ch.~8) provides a
...
on samples of moderate size \cite{Uchaikin2003}.

Barunik and Vacha \cite{Barunik2010} provided Monte-Carlo evidence comparing Hill, Pickands,
...
distributions are available \cite{Nolan2001} and have been shown to be
asymptotically efficient \cite{DuMoutierde1983}. Characteristic-function
(CF) based estimators, including those of Feuerverger and McDunnough \cite{Feuerverger1981} and
Press \cite{Press1972}, exploit the analytic form of the CF. Both families are
...
\cite{Gopikrishnan1999}, which would place the tail exponent outside the
Lévy-stable regime. Weron \cite{Weron2001} showed definitively that the Hill
...
The reconciliation proposed by Weron \cite{Weron2001} and adopted here is that
...
Lo and MacKinlay \cite{LoMacKinlay1988} introduced the variance-ratio test
\begin{equation}
...
appears in Campbell, Lo, and MacKinlay \cite{CampbellLoMacKinlay1997}, Chapter~2.
...
horizons by Lo and MacKinlay \cite{LoMacKinlay1988}. Several mechanisms have been proposed:

\textbf{Bid-ask bounce.} Roll \cite{Roll1984} showed that even in the absence
...
\textbf{Order-book pressure and price reversion.} Bouchaud et al.\ \cite{Bouchaud2004}
and Farmer et al.\ \cite{Farmer2006} documented that large market orders move prices, but
...
comes from Lo and MacKinlay \cite{LoMacKinlay1990}, who found stronger mean reversion in
small-capitalization stocks. For index futures specifically,
Bouchaud and Potters \cite{BouchaudPotters2003} (Ch.~6) report first-order autocorrelations
...
the second-cumulant correction of Bouchaud and Potters \cite{BouchaudPotters2003}, Ch.~6).
...
and empirically validated in the prior literature. Bouchaud and Potters \cite{BouchaudPotters2003}
discuss the qualitative direction of the effect, and
Samorodnitsky and Taqqu \cite{SamorodTaqqu1994} (Section~7.2) derive the exact scale modification
...
Chow and Denning \cite{ChowDenning1993} extended the Lo and MacKinlay \cite{LoMacKinlay1988} test to a
joint test over multiple lags using the studentized maximum modulus distribution.
Wright \cite{Wright2000} proposed rank and sign-based variance-ratio tests with
better finite-sample size. Richardson and Stock \cite{Richardson1991} derived the exact
...
the point-estimator interpretation of Campbell, Lo, and MacKinlay \cite{CampbellLoMacKinlay1997}.
...
hedging). This ``U-shape'' was documented early by Wood, McInish, and Ord \cite{WoodMcInishOrd1985}
...
Andersen and Bollerslev \cite{AndersenBollerslev1997,AndersenBollerslev1998}, who showed that the
...
Andersen and Bollerslev \cite{AndersenBollerslev1998} applied this decomposition to Deutsche
...
specifically, Ederington and Lee \cite{Ederington1993} documented U-shaped volatility patterns
in S\&P 500 futures and Lee \cite{Lee1994} showed that de-seasonalization is
...
Bouchaud and Potters \cite{BouchaudPotters2003} (Ch.~6) discuss the de-seasonalization step as
...
Kadioglu et al.\ \cite{Kadioglu2023} applied recurrence quantification analysis to intraday
...
for high-frequency modeling \cite{ParkEomKim2022,LiangWan2023}.
...
the intermittency of velocity increments \cite{Kolmogorov1941}. The same
framework was imported into finance by Muzy and Bacry \cite{MuzyBacry1993,MuzyBacry1994}
and Mandelbrot \cite{Mandelbrot1997} to test for simple versus multifractal scaling.
...
Calvet and Fisher \cite{Calvet1997} and Mandelbrot \cite{Mandelbrot1997} provided early evidence
for multifractal scaling in FX data, and the multifractal random walk model of
Bacry and Muzy \cite{BacryMuzy2003} has become influential in the volatility-modeling
literature. However, the evidence is contested: Cont \cite{Cont2001} notes that
apparent multifractality can arise from volatility clustering (GARCH effects)
in finite samples, and Eisler et al.\ \cite{Eisler2004} showed that multifractal estimates
from individual stocks are sensitive to the choice of $q$ values and sample length.

More recently, Saadaoui et al.\ \cite{Saadaoui2024} applied multifractal detrended
...
Morales et al.\ \cite{Morales2012} provided a detailed comparison of scaling exponent
...
\cite{Cont2001,Bouchaud2001}. At intraday horizons the direction of
asymmetry is less settled, but Aït-Sahalia et al.\ \cite{Ait-Sahalia2002} found statistically
significant skewness in high-frequency equity returns, while
Bouchaud and Potters \cite{BouchaudPotters2003} (Ch.~3) argue that the skewness at 1-minute
...
literature \cite{Zolotarev1986,Nolan2001}.
...
and methodological choices has received growing attention. Cont \cite{Cont2001}
emphasized that different estimators can give qualitatively different answers
for the same dataset. Weron \cite{Weron2001} provided Monte Carlo evidence on the
bias and variance of common estimators. Barunik and Vacha \cite{Barunik2010} compared
...
Sala-i-Martin \cite{Sala-i-Martin1997}'s sensitivity analysis methodology. To our
...
TLF framework of Mantegna and Stanley \cite{MantegnaStanley1994,MantegnaStanley1995} to a
...
combinations. The comparison addresses the cautions of Weron \cite{Weron2001}
and Barunik and Vacha \cite{Barunik2010} about relying on a single estimator.
...
with the interpretation in Bouchaud and Potters \cite{BouchaudPotters2003}.
