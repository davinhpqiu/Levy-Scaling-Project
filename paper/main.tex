...
\State Regress $\log\Ph(0,\tau)$ on $\log\tau$;\; extract slope $\hat{m}$
...
\For{each $q\in\{0.25,\;0.50,\;0.75,\;1.00,\;1.25,\;1.50\}$}
  \State $\hat{S}_q(\tau)\leftarrow N_\tau^{-1}\sum_t|\delta_\tau(t)|^q$;\; regress on $\log\tau$;\; slope $\hat\zeta(q)$
...
\For{each $\alpha\in[1.2,\;2.8]$ on a grid of 160 values}
  \State Rescale: $\tilde{P}(\tilde{x},\tau)\leftarrow\tau^{1/\alpha}\Ph\!\left(\tau^{1/\alpha}\tilde{x},\,\tau\right)$
  \State Compute score $\mathrm{Score}(\alpha)$ per Eq.~\eqref{eq:score}
\EndFor
...
\State Estimate $\rhoh(k)$ from 1-min increments, $k=1,\ldots,K_{\max}$
\State Build $\VRh(\tau)$ per Eq.~\eqref{eq:vr_hat};\; set $b(\tau)\leftarrow-\tfrac{1}{2}\log\VRh(\tau)$
\State Regress $\log\Ph(0,\tau)-b(\tau)$ on $\log\tau$ (coeff.\ of $b(\tau)$ fixed at 1);\; slope $\hat{m}_{\mathrm{corr}}$
