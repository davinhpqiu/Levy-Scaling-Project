...
\begin{table}[H]
  ...
  Means over $\varepsilon\in\{0.1,\ldots,0.5\}$ and $N_\tau\in\{20,37\}$.
  $\Delta R^2 = R^2_{\mathrm{des}} - R^2_{\mathrm{raw}}$.
\end{table}
...
\begin{table}[H]
  ...
  \smallskip\footnotesize
  Means over $\varepsilon\in\{0.1,\ldots,0.5\}$ and $N_\tau\in\{20,37\}$, raw series.
  Spread $=\alphah_{\mathrm{collapse}} - \alphah_{\mathrm{tip}}$.
\end{table}
...
\begin{table}[H]
  ...
  \smallskip\footnotesize
  Averaged over $x>0$ and all lags in the grid, at $\varepsilon=0.1$,
  $N_\tau=20$, raw series.
\end{table}
...
\begin{table}[H]
  ...
  \smallskip\footnotesize
  Sorted by $\VRh_\infty$ ascending. Estimates at $\varepsilon=0.1$,
  $N_\tau=20$, raw series, $K_{\max}=60$.
  $\alphah_{\mathrm{tip}}$ and $\alphah_{\mathrm{corr}}$ are from the
  tick-scaled peak-scaling pipeline used throughout Sections I and VI of the
  analysis; values for ES, NQ, and PT reflect the higher x-grid convention
  used in that pipeline and should be compared within-column rather than
  to Table~\ref{tab:three_way}.
\end{table}
