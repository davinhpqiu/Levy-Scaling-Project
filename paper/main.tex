...
classes---foreign exchange (FX) and sovereign fixed income---for three
reasons.
...
driven by a single dominant risk factor---aggregate equity market
...
equally spaced bin centres spanning $[-x_{\max},\,x_{\max}]$, which define
...
density of the bin whose centre is nearest $x = 0$.
...
\For{each $\tau\in\mathcal{T}$}
  ...
  \State Grid: $N_x+1=2{,}001$ bin centres spanning $[-x_{\max},x_{\max}]$
