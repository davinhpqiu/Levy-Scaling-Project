...
PT & S\&P/TSX 60           & Americas & MX (Montr\'{e}al) & Oct 1999    & 09:30--16:15 \\
...
Two distinct conditions must hold for 1-minute returns to carry genuine price
information, and Table~\ref{tab:sigma} reports both.
The first concerns \textit{resolution}: no price change smaller than one tick
is observable, so $\sigma_1/\Delta > 1$ is required for the typical 1-minute
move to exceed the quotation grid. The ratio ranges from $2.71$ (ES) to
$9.31$ (NQ).
The second concerns the \textit{bid-ask bounce}: when successive trades
arrive alternately at the bid and the ask, they generate a spurious price
change of one effective spread $s$ with no movement in the underlying price
\cite{Roll1984}. Genuine price variation therefore has to dominate the bounce
amplitude, which requires $\sigma_1/s > 1$. This ratio is the tighter of the
two, ranging from $1.11$ (FT) to $4.90$ (NQ).
Both conditions are satisfied by all eight contracts, confirming that the
1-minute frequency is informative across the full cross-section. The bounce
mechanism is not merely a screening criterion here: it is the source of the
short-term mean reversion for which Section~\ref{sec:background:vr} derives an
explicit correction.
Note that the tick size, not the spread, sets the resolution of the histogram
grid used throughout Section~\ref{sec:methodology}, where the bin width is
defined as a multiple of $\Delta$.
...
\multicolumn{6}{l}{\small $s$ is the average quoted bid-ask spread.}\\
\end{tabular}
\label{tab:sigma}
\end{center}
\end{table}
...
\textbf{Integer-scale conversion.} To work in integer arithmetic and avoid
...
integer-valued. The tick size $\Delta$ enters separately, through the
...
\begin{equation}
  x_{\max} = \Delta\cdot N_x\cdot
             \max\!\left(\left\lfloor\varepsilon\,\frac{\sigma_1}{\Delta}\right\rceil,\,1\right),
  \label{eq:xmax}
\end{equation}
where $\Delta$ is the contract tick size (Table~\ref{tab:sigma}), $\sigma_1$
is the standard deviation of 1-minute price changes, $\lfloor\cdot\rceil$
denotes rounding to the nearest integer, and $\varepsilon$ is a dimensionless
resolution parameter. The resulting bin width is
$\Delta x = 2x_{\max}/N_x = 2\Delta\lfloor\varepsilon\sigma_1/\Delta\rceil$,
so bin edges always fall on integer multiples of the tick and no bin can
straddle a fraction of a quotable increment.
...
At $\varepsilon=0.1$ the rounded factor equals 1 for all eight markets, giving
a bin half-width of exactly one tick; below this, counts in the central bin
become too sparse at short lags to estimate $\Ph(0,\tau)$ reliably. At
$\varepsilon=0.5$ the factor reaches 2--3 and the wider bins begin to smooth
...
$\alphah_{\mathrm{collapse}}$ is the minimizer of Score($\alpha$) over a grid
...
\label{alg:pipeline}
\begin{algorithmic}[1]
\Require Price series $\{P(t)\}$, tick size $\Delta$, decimal multiplier $M$,
         1-minute volatility $\sigma_1$,
...
\State Integer-scale prices:\; $p(t)\leftarrow\mathrm{round}(P(t)\cdot M)$, $M$ the decimal multiplier
...
\For{each $\tau\in\mathcal{T}$}
  ...
  \State $x_{\max}\leftarrow\Delta N_x\max(\lfloor\varepsilon\sigma_1/\Delta\rceil,1)$;\; $\Delta x\leftarrow 2x_{\max}/N_x$
  \State Grid: $N_x+1=2{,}001$ bin centers spanning $[-x_{\max},x_{\max}]$
  \State Build normalized histogram $\Ph(x_j,\tau)$ per Eq.~\eqref{eq:pdf_hat}
...
Despite this heterogeneity, both informativeness ratios $\sigma_1/\Delta$ and
$\sigma_1/s$ remain above unity for every contract
...
Three independent estimators (tip-scaling OLS, collapse quality minimization,
...
Symmetrizing the PDFs prior to collapse matching produces at most a marginal
...
mean-reverting in the long run ($\VRh_\infty=0.958<1$) and therefore
