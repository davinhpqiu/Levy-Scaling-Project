...
amplitude, which requires $\sigma_1/s > 1$. This ratio ranges from $1.11$ (FT) to $4.90$ (NQ).
Both conditions are satisfied by all eight contracts, hence the
1-minute frequency is informative across the full cross-section. Note that the bounce
mechanism is the source of the
short-term mean reversion for which Section~\ref{sec:background:vr} derives an
explicit correction.
Further note that tick size, not the spread, sets the resolution of the histogram
