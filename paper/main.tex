...
Note that the $\alphah_{\mathrm{tip}}$ values for ES, NQ, and PT in this table
are drawn from the tick-scaled peak-scaling pipeline (Section~\ref{sec:methodology:bias}),
which uses a price-grid-relative $x_{\max}$ and produces larger raw estimates
than the unit-normalised pipeline used in Tables~\ref{tab:seasonality}--\ref{tab:asymmetry}.
The bias-correction procedure is applied consistently within this pipeline;
the corrected values $\alphah_{\mathrm{corr}}$ and the correction magnitudes
$\widehat{\Delta\alpha}$ are the quantities of primary interest.
