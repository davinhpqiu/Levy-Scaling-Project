...
truncated Lévy-flight consensus, spanning $\alphah_{\mathrm{tip}}\in(1.72,\,1.83)$
...
\cite{Roll1984}. The ratio exceeds 1 for seven of the eight markets,
confirming that the 1-minute frequency is appropriate for those contracts.
The S\&P/TSX~60 futures contract (PT) is the exception with $\sigma_1/\Delta=0.18$,
indicating that the bid-ask spread of CAD~2.50 substantially exceeds the
typical 1-minute price movement; this is discussed further in
...
interval $(1.72,\,1.83)$, consistent with the truncated Lévy flight picture of
...
empirical PDF peak is a very accurate description of the data across the full cross-section.
...
The S\&P/TSX~60 futures contract (PT) presents a noteworthy microstructure
profile despite yielding a normal $\alphah_{\mathrm{tip}}\approx 1.73$ with
$R^2\approx 0.997$.
Its $\sigma_1/\Delta$ ratio of $0.18$ (Table~\ref{tab:sigma}) is the only
ratio below the threshold of 1 in the sample, indicating that the average
bid-ask spread of CAD~2.50 substantially exceeds the typical 1-minute price
movement of $\sigma_1\approx 0.46$ index points.
This places PT in a microstructure regime where a large fraction of 1-minute
intervals record a zero price change; nevertheless, once the histogram grid
width is calibrated to the actual tick size (the minimum price increment of
CAD~0.10) rather than the bid-ask spread, the peak-scaling regression recovers
a Lévy exponent consistent with the rest of the cross-section.
This result illustrates that proper histogram-width calibration can extract
Lévy scaling even from markets with low 1-minute informativeness.
...
to $+0.095$ (FT), indicating that de-seasonalization consistently raises $\alphah$.
...
hypothesis that intraday volatility clustering concentrates mass at zero-lag
increments in the raw series, mildly flattening the peak-scaling slope.
...
Across all eight markets the inter-estimator spreads are comparable (0.35--0.55 units),
...
\hline
CF & 1.770 & 0.997 & 1.890 & 2.320 & 0.550 \\
DA & 1.718 & 0.997 & 1.862 & 2.180 & 0.462 \\
ER & 1.747 & 0.997 & 1.898 & 2.230 & 0.483 \\
ES & 1.784 & 0.997 & 1.883 & 2.190 & 0.406 \\
FT & 1.821 & 0.998 & 1.931 & 2.350 & 0.529 \\
NQ & 1.779 & 0.997 & 1.861 & 2.130 & 0.351 \\
PT & 1.727 & 0.997 & 1.841 & 2.260 & 0.533 \\
YM & 1.760 & 0.998 & 1.882 & 2.150 & 0.390 \\
\hline
...
five resolution values and two grid densities is below 0.035 for every market
and estimator.
...
fitted exponents across estimators differ by 0.35--0.55 units.
This inter-estimator gap is substantially larger than the inter-market
spread ($\alphah_{\mathrm{tip}}$ ranges from 1.72 to 1.82 across all eight markets),
which implies that reported $\alphah$ values from different
...
The correction is positive (downward) for all eight markets, and the markets with
the smallest $\VRh_\infty$ (strongest net mean reversion) display the largest
corrections.
PT, which has $\VRh_\infty = 0.895$ and $\rhoh(1) = -0.020$, is the most
mean-reverting contract and shows the largest downward correction
($\widehat{\Delta\alpha}\approx 0.014$).
The corrections are uniformly small (at most 0.014 units), reflecting that the
sample means are essentially free of the large discreteness-driven bias that
affects the uncorrected estimator in illiquid markets.
Although ER has a marginally positive first-order autocorrelation
($\rhoh(1) = +0.001$), its long-run variance ratio $\VRh_\infty = 0.958$
...
leaves the ordering unchanged and produces comparable correction magnitudes,
consistent with microstructure effects at very short horizons being modest.
The $K_{\max}$ sensitivity analysis on the pilot market (ES) confirms that
$\alphah_{\mathrm{corr}}$ is stable across a range of truncation choices,
...
\hline
\multicolumn{6}{l}{Single configuration $\varepsilon=0.1$, $N_\tau=20$, $K_{\max}=60$, not averaged across settings; sorted by $\VRh_\infty\uparrow$. See Tables~\ref{tab:seasonality}--\ref{tab:three_way} for averaged values.}
\end{tabular}
\label{tab:bias}
\end{center}
\end{table}
...
instruments.
The S\&P/TSX 60 contract (PT), despite having a $\sigma_1/\Delta$ ratio of $0.18$
(the only contract below the threshold of 1), yields $\alphah_{\mathrm{tip}}\approx 1.73$
and $R^2\approx 0.997$ once the histogram grid is calibrated to the tick size rather
than the bid-ask spread, demonstrating that proper binning recovers Lévy scaling
even in illiquid microstructure regimes.
...
$\alphah_{\mathrm{tip}}$ by $\widehat{\Delta\alpha}\in(0.003,\,0.014)$, with
the correction monotonically ordered by $\VRh_\infty$, a cross-sectional
validation of the bias mechanism.
ER, though it has marginally positive $\rhoh(1)=+0.001$, has $\VRh_\infty=0.958<1$
and corrects downward by $\widehat{\Delta\alpha}=0.007$.
The corrected estimates fall within the interval $(1.69,\,1.80)$,
