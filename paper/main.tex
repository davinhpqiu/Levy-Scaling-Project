...
\end{equation}
The resulting slope bias is
$\mathrm{Cov}(\log\tau,\,-\tfrac12\log VR(\tau))/\mathrm{Var}(\log\tau)$,
so its direction is governed by how $VR(\tau)$ varies with the aggregation
horizon rather than by its level: a constant variance ratio, however far below
unity, shifts only the intercept. In the estimated profiles $VR(\tau)$
declines with $\tau$, so $-\tfrac12\log VR(\tau)$ is positively associated
with $\log\tau$; omitting it makes the raw OLS slope less negative and biases
...
$\alphah_{\mathrm{tip}}$ by $0.082$--$0.153$ units (mean $0.119$), a gap
...
roughly two thirds separates the collapse criterion from the two
...
In unreported checks the symmetrized collapse, in which the empirical PDF is
replaced by $\Ph_{\mathrm{sym}}(x,\tau) = [\Ph(x,\tau)+\Ph(-x,\tau)]/2$,
matches or marginally improves on the raw collapse for all markets.
...
A $K_{\max}$ sensitivity analysis on the pilot market (ES) leaves
$\alphah_{\mathrm{corr}}$ stable across truncation choices,
...
regression-based methods are broadly consistent, differing by $0.082$--$0.153$
