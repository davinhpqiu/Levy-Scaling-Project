...
\usepackage{standalone} % allows \input{pipeline_figure} when ready
...
\caption{PDF collapse for all eight markets ($\varepsilon=0.3$, $N_\tau=20$).
Each panel shows the rescaled density $\tau^{1/\hat\alpha}\hat{P}(x,\tau)$
against the rescaled coordinate $\tilde{x}=x/\tau^{1/\hat\alpha}$, with
$\hat\alpha=\hat\alpha_{\mathrm{collapse}}$ for that market.
The title of each panel reports the fitted exponent.}
\label{fig:collapse}
\end{figure*}
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_three_estimators}
\caption{Lévy exponent estimates from three independent estimators for each
market. Error bars show the standard deviation across resolution and grid
settings ($\varepsilon\in\{0.1,\ldots,0.5\}$, $N_\tau\in\{20,37\}$).
The dashed line marks the Gaussian boundary $\alpha=2$.}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.92\textwidth]{figures/fig_bias_correction}
\caption{\textit{Left:} $\hat\alpha_{\mathrm{tip}}$ (circles) and
$\hat\alpha_{\mathrm{corr}}$ (crosses) plotted against $\widehat{VR}_\infty$
for each market; vertical segments show the correction magnitude.
\textit{Right:} $\widehat{\Delta\alpha}=\hat\alpha_{\mathrm{tip}}-\hat\alpha_{\mathrm{corr}}$
ordered by $\widehat{VR}_\infty$ ascending.
The sole negative bar (ER) corresponds to the only market with
$\hat\rho(1)>0$.}
