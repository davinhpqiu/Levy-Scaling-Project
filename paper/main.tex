...
\begin{document}git add paper/main.tex paper/references.bib paper/main.bbl
git commit -m "Fix citation attributions and systematic hyphenation audit"
...
concatenated on a continuous session-minute index, so consecutive trading days
are adjacent in the series and each day-to-day transition contributes one
increment at every lag; overnight price changes therefore enter the $\tau=1$
distribution as single-minute increments. The robustness check excluding
$\tau\in\{1,2,3\}$ (Section~\ref{sec:results:bias}) bounds the influence of
this treatment.
...
(Table~\ref{tab:sigma}), $\sigma_1$ is the standard deviation of 1-minute
price changes, $\lfloor\cdot\rceil$
...
(for ES and PT only after the unit floor is applied), giving a bin half-width
of exactly one tick; below this, counts in the central bin become too sparse
...
falls only from $n$ to $n-\tau$ across the grid, the number of effectively
independent increments falls as $n/\tau$: at the longest lag it is of order
$40$ per market. The regression is unweighted, so the shortest and longest
...
sample, with $12.5\%$ of its session-minute grid supplied by gap-filling
...
($\rhoh(1)=+0.001\approx 0$), but its long-run variance ratio $\VRh_{60}=0.958<1$
...
($\rhoh(1) = +0.001$), its long-run variance ratio $\VRh_{60} = 0.958$
...
Three distinct estimators (tip scaling OLS, collapse quality minimization,
...
mean reverting in the long run ($\VRh_{60}=0.958<1$) and therefore
corrects downward like the rest.
Long-run variance ratios span $\VRh_{60}\in[0.895,\,0.980]$.
