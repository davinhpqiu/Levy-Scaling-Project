...
% ── Title block ───────────────────────────────────────────────────────────────
\title{Empirical L\'{e}vy Scaling in Short-Horizon Futures:
...
variance ratio, intraday returns.
\end{IEEEkeywords}
...
simple Lévy scaling \cite{Mandelbrot1997,MuzyBacry1993,Jiang2019}.
...
pattern \cite{WoodMcInishOrd1985,AndersenBollerslev1997,KimShin2023}. The
...
robustness check.
Algorithm~\ref{alg:pipeline} summarises the complete six-step estimation
pipeline, from preprocessing through bias-corrected tip estimation.

% ── Estimation pipeline (end of methodology) ─────────────────────────────────
% \clearpage flushes pending floats and starts a fresh page so the algorithm
% lands at the top of the left column rather than mid-column or spanning both.
\clearpage
\begin{algorithm}[t]
...
\State Estimate volatility profile: $\hat{\sigma}_m\leftarrow D_m^{-1}\sum_{d:\,m(k)=m}|\delta_k|$;\; normalize so $\bar{\hat\sigma}=1$
...
\For{each $\tau\in\mathcal{T}$}
  ...
  \State Bin width $\Delta x=\varepsilon\cdot\Delta$;\; $N_x=2{,}000$ bins centred at $x=0$
  \State Build normalised histogram $\Ph(x_j,\tau)$ per Eq.~\eqref{eq:pdf_hat}
...
\For{each $q\in\{0.25,\;0.50,\;0.75,\;1.00,\;1.25,\;1.50\}$}
  \State $\hat{S}_q(\tau)\leftarrow N_\tau^{-1}\sum_t|\delta_\tau(t)|^q$;\; regress on $\log\tau$;\; $\alphah_q\leftarrow q/\hat\zeta(q)$
\EndFor
...
\For{each $\alpha\in[1.2,\;2.8]$ on a grid of 160 values}
...
\State Estimate $\rhoh(k)$ from 1-min increments, $k=1,\ldots,K_{\max}$;\; build $\VRh(\tau)$ per Eq.~\eqref{eq:vr_hat}
\State Set $b(\tau)\leftarrow-\tfrac{1}{2}\log\VRh(\tau)$;\; regress $\log\Ph(0,\tau)-b(\tau)$ on $\log\tau$ (coeff.\ of $b(\tau)$ fixed at 1)
...
% ══════════════════════════════════════════════════════════════════════════════
% ── Bias correction figure (placed after conclusion to appear on final page) ──
\begin{figure*}[!t]
\centering
\includegraphics[width=0.92\textwidth]{figures/fig_bias_correction.png}
\caption{Variance-Ratio Bias Correction by Market. Each panel shows the raw
log~$\hat{P}(0,\tau)$ vs.\ $\log\tau$ (dots) and the bias-corrected regression
line (solid), for each of the eight equity-index futures contracts.}
