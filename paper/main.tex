...
\Ensure $\alphah_{\mathrm{tip}}$,\; $\alphah_{\mathrm{sf}}$,\; $\alphah_{\mathrm{collapse}}$,\; $\alphah_{\mathrm{corr}}$
\Statex
...
\For{each $\tau\in\mathcal{T}$}
  ...
  \State $x_{\max}\leftarrow\Delta N_x\max(\lfloor\varepsilon\sigma_1/\Delta\rceil,1)$;\; $\Delta x\leftarrow 2x_{\max}/N_x$;\; $N_x{+}1$ bin centers on $[-x_{\max},x_{\max}]$
