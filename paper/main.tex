...
spanning North America and Europe, using 1-minute closing prices from the start
...
central concern of financial economics and econophysics for over a century.
The Gaussian random-walk model, which predicts
...
geographies and regulatory environments. The canonical central-body estimates
also rest on samples drawn from the 1980s and 1990s. Recent work confirms that
heavy-tailed, non-Gaussian intraday distributions persist
\cite{Masoliver2021,ContMiami2023}, but to our knowledge the scaling exponents
themselves have not been re-measured on contemporary data. Second, the sensitivity of $\alphah$ to methodological
...
This paper addresses all three gaps. We use 1-minute closing prices for eight
...
interpretation of $\alphah$ as a central-body parameter. More recent work
supports continued application of this framework to contemporary data:
W{\k{a}}torek et al.\ \cite{Masoliver2021} document persistent heavy-tailed,
non-Gaussian return distributions through the COVID period, and Ratliff-Crain
et al.\ \cite{ContMiami2023} find Cont's stylized facts largely intact in
2018--2019 intraday US equity data.
...
moderate samples \cite{Nolan2020}, while $q\in(1,\alpha)$ are
...
1-minute OHLCV bars were obtained from TickData (Tick Data, LLC) and processed
using the OneTick tick analytics platform, covering each market from the start
of available data through 14 January 2026. Only the closing price of each
1-minute bar enters the analysis; the open, high, low and volume fields are
not used. The files are supplied as
...
\cite{Roll1984}; estimating that spread from discretely observed prices
remains an active problem \cite{Ardia2024,Shaik2026}. Genuine price variation
must therefore dominate the bounce
...
in the return distribution that distorts its high-frequency scaling;
Nord{\'e}n et al.\ \cite{Norden2026} document the same tick and lot size
constraints acting on liquidity in futures markets specifically. This
...
\hline
\multicolumn{6}{@{}l@{}}{\small $\sigma_1$, $\Delta$ (tick) and $s$ (spread) in index points.}\\
\multicolumn{6}{@{}l@{}}{\small $\sigma_1$ is the reference calibration value for the}\\
\multicolumn{6}{@{}l@{}}{\small histogram grid, not a whole-sample estimate.}\\
\end{tabular}
\label{tab:sigma}
\end{center}
\end{table}
...
$0.2\%$ at $\tau=1$ to unity at one full session, where $J$ denotes the
session length in minutes ($405$ for the five North American contracts, $510$
for the three European ones). The identically distributed assumption
underlying Equation~\eqref{eq:scaling} therefore holds only approximately. At lags
...
pattern \cite{AndersenBollerslev1997,Chakraborti2011}, a
feature that remains central to contemporary intraday volatility
modelling \cite{Liang2023,Zhang2024}. Hua \cite{Hua2025} surveys intraday
seasonality alongside nonstationarity and low signal-to-noise ratios as the
principal obstacles in contemporary high-frequency data analysis. The
...
minute-of-session, $J$ is the session length defined in
Section~\ref{sec:data:preprocessing}, and $\hat\sigma_m$ is the deterministic
intraday factor. The
...
The four US equity-index futures (YM, ER, NQ, and ES), with
...
Figure~\ref{fig:collapse} shows the corresponding rescaled densities: for
every market the curves for different lags fall close to a common shape over
the plotted range, which is the qualitative signature the peak scaling
...
Figure~\ref{fig:three_estimators} and Table~\ref{tab:three_way} compare
$\alphah_{\mathrm{tip}}$,
...
Figure~\ref{fig:bias_correction} summarizes the correction across markets.
...
1-minute closing prices from the start of available data through January 2026.
Six findings emerge.
