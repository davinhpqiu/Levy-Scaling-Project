...
Table~\ref{tab:sigma} reports, for each market, the standard deviation of
1-minute price changes $\sigma_1$, the average effective bid-ask spread $s$,
...
The bounce is not merely a screening criterion here: it is the mechanism
generating the short-term mean reversion for which
Section~\ref{sec:background:vr} derives an explicit correction, and the
markets with the smallest $\sigma_1/s$ are those for which that correction
matters most.
The tick size is reported for a separate reason: it sets the resolution of
...
in the return distribution that distorts its high-frequency scaling, an
effect that attenuates as the observation scale increases and therefore acts
on the lag dependence that the peak scaling estimator exploits. All eight
contracts here sit in the small-tick regime, with $\sigma_1$ exceeding
$\Delta$ by factors of $2.7$ to $9.3$, so the increment distributions are not
dominated by an atom at the origin and $\Ph(0,\tau)$ is estimated from a
genuinely populated central bin.
...
Despite this heterogeneity, the informativeness ratio $\sigma_1/s$ remains
