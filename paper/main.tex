...
cross-sectionally: the correction magnitude is strongly negatively
associated with the variance ratio across markets, providing empirical
...
changes $\sigma_1$ and the tick size $\Delta$ (the minimum quotable price
increment) for each market.
A necessary condition for 1-minute returns to carry genuine price information
is $\sigma_1 / \Delta > 1$; otherwise the typical 1-minute move is smaller
than the smallest observable price change, and a substantial fraction of
recorded increments reflects price discreteness and the bid-ask bounce
rather than true price movement \cite{Roll1984}. The ratio exceeds 1 for all
eight markets, ranging from $2.71$ (ES) to $9.31$ (NQ), confirming that the
1-minute frequency is informative across the full cross-section.
The tick size also sets the resolution of the histogram grid used throughout
Section~\ref{sec:methodology}, where the bin width is defined as a multiple
of $\Delta$.

\begin{table}[!t]
\begin{center}
\caption{Market Volatility and Tick Size}
\begin{tabular}{lrrr}
\hline
Market & $\sigma_1$ & $\Delta$ (tick) & $\sigma_1/\Delta$ \\
\hline
YM & 6.15 & 1.00 & 6.15 \\
NQ & 2.33 & 0.25 & 9.31 \\
ER & 0.51 & 0.10 & 5.11 \\
...
\cdot \Delta$, where $\Delta$ is the contract tick size (Table~\ref{tab:sigma})
and $\varepsilon$ is a dimensionless resolution parameter. Defining the bin
width relative to the tick guarantees that bin edges fall on observable price
levels, so that no bin can straddle a fraction of a quotable increment.
The normalised histogram density is
\begin{equation}
  \Ph(x_j,\tau)
  = \frac{\#\{i: \Delta P_\tau(t_i)\in\text{bin }j\}}{M_\tau\cdot\Delta x},
  \label{eq:pdf_hat}
\end{equation}
where $M_\tau$ is the number of available increments at lag $\tau$ (distinct from the lag-grid density $N_\tau$). The bin half-width
...
common evaluation grid in $\tilde{x}$, shared across all lags so that $\mu_j$
is well defined. Rescaled densities are interpolated linearly onto
$\mathcal{G}$ and grid points falling outside the observed support at a given
lag are excluded from the sum.
Because $\tilde{x}$ inherits the contract's price scale through $x_{\max}$,
which varies by two orders of magnitude across the cross-section, a grid
fixed in absolute price units would sample a different portion of the
...
relative to each market's own histogram half-width: 801 equally spaced points
on $[-x_{\max}/10,\,x_{\max}/10]$. The collapse estimator
$\alphah_{\mathrm{collapse}}$ is the minimiser of Score($\alpha$) over a grid
of 161 candidate values spanning $[1.2,2.8]$ in steps of $0.01$.
...
Despite this heterogeneity, the informativeness ratio $\sigma_1/\Delta$
remains comfortably above unity for every contract
(Table~\ref{tab:sigma}), and the fitted exponents are confined to a narrow
band.
The S\&P/TSX~60 contract (PT) is the least actively traded instrument in the
sample and has the smallest $\sigma_1$ in absolute terms, yet once the
...
This illustrates that tick-relative histogram calibration, rather than any
absolute price scale, is what makes the estimates comparable across markets
...
Second, every estimated value ($\alphah_{\mathrm{collapse}}
...
estimated exponents across estimators differ by 0.23--0.36 units.
...
$\VRh_\infty$, with a Pearson correlation of $-0.98$ between the two
quantities: the markets furthest from the martingale benchmark receive the
largest corrections, and those closest to it the smallest. The ordering is
not strictly monotone at the market level, as the four contracts with
$\VRh_\infty$ between $0.949$ and $0.958$ are separated by less than
$0.01$ in variance ratio and their corrections differ by under
$0.002$ exponent units, which is within the estimation noise of
$\widehat{\Delta\alpha}$. The systematic negative association nonetheless
constitutes a cross-sectional validation of the bias mechanism derived in
...
$\VRh_\infty$ (Pearson $-0.98$), a cross-sectional validation of the bias
mechanism.
